...
class BaseServiceModel(BaseAccountModel):
    server = models.ForeignKey(to=Server, on_delete=models.CASCADE, verbose_name="服务器", null=True, blank=False, db_index=True)
...
class BaseServiceUserModel(BaseModel):
    ...
    server = models.ForeignKey(to=BaseServiceModel, on_delete=models.CASCADE, verbose_name="服务", null=True, blank=False)
