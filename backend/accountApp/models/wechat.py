...
class Wechat(BaseModel):
    id = models.CharField(verbose_name="微信号（ID）", primary_key=True, max_length=20, unique=True, null=False, blank=False, db_index=True)
    nickName = models.CharField(max_length=20, null=True, blank=True, db_index=True, verbose_name='微信名（可忽略）')
    ...
    email = models.OneToOneField(verbose_name="绑定的邮箱", to=Email, on_delete=models.CASCADE, null=True, blank=True, db_index=True, help_text="你可以使用一验证过的邮箱地址登陆微信，也可以用它来找回微信密码")
    ...
    def getRemark(self):
        if self.remark is None:
            ...
            return "%s(%s)" % (self.id, self.remark)
